...
export class OfflineRenderer {
  ...
  async renderFrame(request: OfflineFrameRequest, time: number): Promise<Uint8Array> {
    ...
    const waveform =
      request.fractalId === "feast" && request.audioAnalysis
        ? waveformWindowAt(request.audioAnalysis, time)
        : undefined;
    const spectrum =
      request.fractalId === "bars" && request.audioAnalysis
        ? spectrumWindowAt(request.audioAnalysis, time)
        : undefined;
    this.renderer.render(request.fractalId, uniforms, waveform, spectrum);
